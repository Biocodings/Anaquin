...
namespace SS
{
    ...
    namespace Internal
    {
        ...
        inline LinearResults lSVD(const Matrix &Y, const Matrix &X)
        {
            ...
            for (auto i = 0; i < B.rows(); i++)
            {
                ...
                c.p = !isinf(c.test) && !isnan(c.test) ? Internal::pval(c.test, Internal::pt(c.test, errorDF), TwoSided) : P(NAN);
                
                c.upper = !isnan(c.test) ? c.est + c.test * Internal::qt(0.95, errorDF) : NAN;
                c.lower = !isnan(c.test) ? c.est - c.test * Internal::qt(0.95, errorDF) : NAN;
            ...
            r.AIC = n * std::log(r.error.ss/n) + 2.0 * r.coeffs.size();
            r.BIC = n * std::log(r.error.ss/n) + std::log(n) * r.coeffs.size();
    ...
    template <typename T, typename... Args> LinearResults linearModel(const T &t, Args... args)
    {
        LinearResults r;
        ...
        Internal::vArgs([&](const std::vector<const T *> &p)
        {
            ...
            Eigen::MatrixXd X(y.size(), p.size());
            X.setConstant(1.0);

            for (auto i = 1; i < p.size(); i++)
            {
                const auto x = *p[i];
                X.col(i) = Eigen::VectorXd::Map(&(x[0]), x.size());
            }
