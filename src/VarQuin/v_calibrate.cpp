...
template <typename Stats> Coverage stats2cov(const VCalibrate::Method meth, const Stats &stats)
{
    switch (meth)
    {
        case VCalibrate::Method::Mean:   { return stats.mean; }
        case VCalibrate::Method::Median: { return stats.p50;  }
        ...
        case VCalibrate::Method::Prop:
        case VCalibrate::Method::Reads: { return stats.mean; }
    }
}

VCalibrate::CalibrateStats VCalibrate::check(const FileName &endo,
                                       const FileName &seqs,
                                       ...
                                       const VCalibrate::Options &o)
{
    ...
    VCalibrate::CalibrateStats stats;
    ...
    for (auto &i : tRegs)
    {
        ...
        for (auto &j : i.second.data())
        {
            ...
            switch (o.meth)
            {
                case VCalibrate::Method::Mean:
                case VCalibrate::Method::Median: { norm = seqsC ? std::min(endoC / seqsC, 1.0) : NAN; break; }
                case VCalibrate::Method::Prop:
                {
                    A_ASSERT(!isnan(o.p));
                    norm = o.p;
...
double VCalibrate::afterSeqsC(const Chr2DInters &tRegs, std::map<ChrID, std::map<Locus, SampledInfo>> &c2v, VCalibrate::Options o)
{
...
VCalibrate::GenomeSequins VCalibrate::tBefore(const CalibrateStats &x, const ParserBAMBED::Stats &y)
{
    VCalibrate::GenomeSequins r;
...
VCalibrate::GenomeSequins VCalibrate::sAfter(const CalibrateStats &x, const ParserBAMBED::Stats &y)
{
    VCalibrate::GenomeSequins r;
...
VCalibrate::GenomeSequins VCalibrate::sBefore(const CalibrateStats &x, const ParserBAMBED::Stats &y)
{
    VCalibrate::GenomeSequins r;
...
VCalibrate::GenomeSequins VCalibrate::tAfter(const CalibrateStats &x, const ParserBAMBED::Stats &y)
{
    VCalibrate::GenomeSequins r;
...
static void generateCSV(const FileName &file, const VCalibrate::Stats &stats, const VCalibrate::Options &o)
{
    o.generate(file);
                            ...
                            const VCalibrate::Options &o)
{
    ...
    const auto summary = "-------VarCalibrate Summary Statistics\n\n"
                         "-------VarCalibrate Inputs\n\n"
...
void VCalibrate::report(const FileName &endo, const FileName &seqs, const Options &o)
{
    const auto stats = analyze(endo, seqs, o);
    
    /*
     * Generating VarCalibrate_summary.stats
     */
    
    generateSummary("VarCalibrate_summary.stats", endo, seqs, stats, o);

    /*
     * Generating VarCalibrate_sequins.tsv
     */

    generateCSV("VarCalibrate_sequins.tsv", stats, o);
}
