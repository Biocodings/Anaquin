    ...
    for (const auto &i : stats.vs)
    {
        if (isSomatic(i.name))
        {
            ...
            o.writer->write((boost::format(format) % i.name
                                                   ...
                                                   % FORMAT_I_("DP_2")
                                                   % r.af(i.name)
                                                   % (isnan(normalAF(i)) ? "-" : toString(normalAF(i)))
                                                   % (isnan(tumorAF(i))  ? "-" : toString(tumorAF(i)))
                                                   % toString(i.qual)
                                                   % ctx2Str(sv->ctx)
                                                   % var2str(i.type())
        ...
        if (isSomatic(i.name))
        {
            ...
            if (isTP)
            {
                ...
                o.writer->write((boost::format(format) % i.name
                                                       ...
                                                       % FORMAT_I("DP_2")
                                                       % r.af(i.name)
                                                       % (isnan(normalAF(c)) ? "-" : toString(normalAF(c)))
                                                       % (isnan(tumorAF(c))  ? "-" : toString(tumorAF(c)))
                                                       % toString(c.qual)
                                                       % ctx2Str(sv.ctx)
                                                       % var2str(i.type())
                                                       % extra(ss, i.key())).str());
    ...
    auto f = [&](const std::vector<VSomatic::Match> &x, const std::string &label)
    {
        for (const auto &i : x)
        {
            ...
            o.writer->write((boost::format(format) % (i.rID.empty() ? "-" : i.rID)
                                                   ...
                                                   % (isnan(normalAF(i.qry)) ? "-" : toString(normalAF(i.qry)))
                                                   % (isnan(tumorAF(i.qry)) ? "-" : toString(tumorAF(i.qry)))
                                                   % toString(i.qry.qual)
                                                   % ctx
                                                   % var2str(i.qry.type())
                                                   % extra(ss, key)).str());
