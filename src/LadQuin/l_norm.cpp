...
LNorm::Stats LNorm::analyze(const FileName &file, const Options &o)
{
...
static void writeCSV(const FileName &file, const LNorm::Stats &stats, const LNorm::Options &o)
{
...
void LNorm::report(const FileName &file, const Options &o)
{
    const auto stats = LNorm::analyze(file, o);
...
//                                                "reads"));
    ...
    o.info("Generating LadNorm_sequins.csv");
    writeCSV("LadNorm_sequins.csv", stats, o);
    ...
    o.writer->write(RWriter::createScript("LadNorm_sequins.csv", PlotLNorm()));
    o.writer->close();
}