...
TransNorm <- function(d, mix=loadMixture(), level='genes', method='neg', spikes=NULL, round=TRUE, k=1, epsilon=1, tolerance=1e-8, isLog=FALSE)
{
    ...
    else if (level == 'isoforms')
    {
        if (method == 'neg')
        {
            ...
            x <- x[rownames(x) %in% row.names(negativeIsoforms(mix)),]
        }
    }
...
.analyzeResult <- function(r, m, p, logFC, units)
{
    ...
    # List of known exon bins
    known <- row.names(m$exons)
    
    # Exon bins that have been detected in the experiment 
    r <- r[rownames(r) %in% known,]
 
    print(sprintf(paste("Detected %d known ", units, sep=''), length(known)))
    print(sprintf(paste("Detected %d experimental ", units, sep=''), length(rownames(r))))
    print(sprintf(paste("%d bins failed to detect ", units, sep=''), length(known) - length(rownames(r))))
    
    #
    # Create a data-frame for all bins defined, whether it's detected. The fold-changes are
    ...
    for (id in known)
    {
        d[id,]$known <- m$exons[id,]$logFold
    }
    
    for (id in rownames(r))
    {
        d[id,]$padj      <- r[id,]$padj
        d[id,]$measured  <- r[id,]$log2fold_B_A
    ...
    m <- lm(known ~ measured, x)
    
    r     <- cor(as.numeric(x$known), as.numeric(x$measured))
...
#
# Analyze a result object generated by DEXSeq
#

.analyzeDEXSeq <- function(r, m, p=0.1, logFC=0)
{
    stopifnot(class(r) == 'DEXSeqResults')

    r <- .analyzeResult(r, m, p, logFC, 'bins')
    r
}
...
.analyzeGLMEdgeR <- function(r, mix, p=0.1, logFC=0, units='genes')
{
    stopifnot(class(r)[1] == 'DGELRT')
    ...
    print(sprintf(paste("Detected %d known ", units, sep=''), length(known)))
    print(sprintf(paste("Detected %d experimental ", units, sep=''), length(rownames(r))))
    print(sprintf(paste("%d genes failed to detect ", units, sep=''), length(known) - length(rownames(r))))
    ...
    for (id in rownames(r))
    {
        d[id,]$padj      <- r[id,]$PValue
        d[id,]$measured  <- r[id,]$logFC
        d[id,]$expressed <- ifelse(r[id,]$PValue <= p, 'T', 'F')
    }
...
.analyzeEdgeR <- function(r, mix, p=0.1, logFC=0)
{
    # List of known sequins
    known <- as.character(mix$genes$ID)
