...
Why do we need sequins?
  ...
  - Determine empharical limit of quantification; below which measurement becomes stochastic  
...
Each row represents a sequin. *Column ID* gives the sequin names, *Length* is the length of the sequins in nucleotide bases, 'MXA' gives the concentration level in attoml/ul.
...
In our second experiment, the weakly expressed isoforms exhibt stochastic behavior and are clearly not linear with the input concentration. Furthermore, there is a limit of quantification (LOQ); below which accuracy of the experiement becomes questionable.
...
To quantify RNA-Seq transcriptome assembly, we need to run a transcriptome assember; i.e., a software that can assembles transcripts and estimates their abundances. Our workflow guide has the details.
...
The fitted logistic curve revels clear relationship between input concentration and sensitivity. Unsurprisingly, the assembler is more sensitive to highly expressed isoforms. The limit-of-assembly (LOA) is defined as the intersection of the curve to sensitivity of 0.70.
...
In this section, we show how to quantify differential expression analysis between expected fold-change and measured fold-change. We apply our method to a data set comparing K562 to GM12878 [1]. Load the dataset:
...
We can also construct a ROC plot. *[1]* for how the true-positives and false-positives are defined.
...
Unsurprisingly, p-value is inverse quadratically related with average counts. All the LFC ratios systematicly outperform LFC 0. The function also estimates the emphricial detection limits, *[1]* has the details.
