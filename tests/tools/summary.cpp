...
struct Data : public std::map<ChrID, ChrData>
{
    ...
    {
        return ::count(*this, [&](const ChrID &cID, const ChrData &x)
        {
...
static std::map<ChrID, ChrData> summaryGTF(const Reader &r)
{
    ...
    for (auto &i : c2d)
    {
        ...
        std::copy(i.second.t2e[i.first].begin(), i.second.t2e[i.first].end(), sorted.begin());
