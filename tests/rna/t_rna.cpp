...
TEST_CASE("T_RNA_Cufflinks")
{
    ...
    const int s1 = parse_options("rna -align tests/data/cufflinks.sam", output, error);
    
    REQUIRE(s1 == 0);
    
    const int s2 = parse_options("rna -model invalid_model -mixture invalid_mixture -align tests/data/cufflinks.sam", output, error);
