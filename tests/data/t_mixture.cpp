...
{
    REQUIRE_THROWS_AS(Standard::instance(true).addRMix(Reader("data/RnaQuin/MRN040_v001.csv")), std::runtime_error);
}

TEST_CASE("Mixture_MRN041")
{
    REQUIRE_THROWS_AS(Standard::instance(true).addRMix(Reader("data/RnaQuin/MRN041_v001.csv")), std::runtime_error);
}
